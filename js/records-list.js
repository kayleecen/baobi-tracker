...
function renderRecordLists(){
  ...
  fl.innerHTML = data.feeds.length===0
    ...
    : data.feeds.map((f,i)=>({f,i})).reverse().map(({f,i})=>
      `<div class="row"><span class="t">${f.at}</span><span class="v">${f.ml} ml</span>
  ...
  dl.innerHTML = data.diapers.length===0 ? ''
    : data.diapers.map((d,i)=>({d,i})).reverse().map(({d,i})=>
      `<div class="row"><span class="t">${d.at}</span>
       <span class="v" style="color:${d.type==='poo'?'var(--peach-d)':'var(--sky-d)'}">${d.type==='poo'?'粑粑':'尿尿'}</span>
